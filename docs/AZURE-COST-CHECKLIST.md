...
| **Container Apps Jobs** (`cron-reminders` `*/15`, `cron-retry-messages` `*/5`, `migrate` manual) | 0.25 vCPU / 0.5 GiB, seconds per run | per vCPU/GiB-second **only while a run executes** (a few seconds each) → negligible | same |
