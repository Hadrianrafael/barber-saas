...
| **Billing SaaS (Stripe)** | Planos como dados (`Plan.limits` JSON, mensal/anual), `/pricing` + `/billing`, Stripe Checkout, webhook idempotente `/api/webhooks/stripe` (`WebhookEvent` de-dup), ciclo `trial → active → past_due+grace → canceled`, ledger `Payment`+`Invoice`, portal do cliente, **gating de plano no servidor** (`assertWithinLimit`/`assertFeature`). |
| **Stripe Connect (pagamento do cliente)** | Onboarding Express + mapeamento de status, links de pagamento (Checkout na conta conectada + application fee `PLATFORM_FEE_BPS`), ledger com `platformFeeCents`/`netCents`, refunds (UI + webhook `charge.refunded`), webhook Connect separado `/api/webhooks/stripe/connect` (segredo próprio, `provider` próprio), 100% isolado do billing SaaS. |
...
|---|---|---|---|
| **Stripe (assinatura SaaS)** | `STRIPE_SECRET_KEY`, `STRIPE_PUBLISHABLE_KEY`, `STRIPE_WEBHOOK_SECRET` | Dashboard Stripe → API keys + Webhooks (endpoint `/api/webhooks/stripe`, eventos `customer.subscription.*`, `invoice.*`). `docs/deployment/stripe.md` | Assinar um plano em `/pricing` → Checkout → confirmar que `/billing` mostra `active` e há linha em `Payment`/`Invoice`. |
| **Stripe Connect (pagamento do cliente)** | `STRIPE_CONNECT_WEBHOOK_SECRET`, `PLATFORM_FEE_BPS` (mesma `STRIPE_SECRET_KEY`) | Stripe → Connect + Webhook separado (endpoint `/api/webhooks/stripe/connect`). `docs/deployment/stripe-connect.md` | `/payments` → conectar conta Express → criar link de pagamento → pagar → conferir `Payment(purpose=CLIENT_PAYMENT)` com `platformFeeCents`/`netCents`. |
